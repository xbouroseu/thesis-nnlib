...
void acc_relu(const Tensor4D<T> &input, Tensor4D<T> &output, Tensor4D<T> &output_drv) {
    int size = input.get_shape().get_size();
    T *in_data = input.get_data(), *out_data = output.get_data(), *out_drv_data = output_drv.get_data();
...
void acc_softmax(const Tensor4D<T> &input, Tensor4D<T> &output) {
    int size = input.get_shape().get_size();
    T *in_data = input.get_data(), *out_data = output.get_data();
    T output_sum = 0.0f;
...
//             output_sum[i] += exp(input[i*num_outputs + j]);
    ...
    for(int j = 0; j < size; j++) {            
        ...
        output.get_data()[j] = exp(in_data[j]) / output_sum;
    }
    }
}
...
void acc_matrix_multiply(const Tensor4D<T> &A, const Tensor4D<T> &B, Tensor4D<T> &C) {
    if(A.get_shape()[2]!=1 && A.get_shape()[3]!=1) {
        throw("Error: Tensor A is not 2D");
    }
    
    if(B.get_shape()[2]!=1 && B.get_shape()[3]!=1) {
        throw("Error: Tensor B is not 2D");
    }
    
    if(A.get_shape()[1] != B.get_shape()[0]) {
        throw("Error: cols(A)!=rows(B). ");
    }
    
    if(A.get_shape()[0] != C.get_shape()[0]) {
        throw("Error: rows(A)!=rows(C)");
    }
    
    if(B.get_shape()[1] != C.get_shape()[1]) {
        throw("Error: cols(B) != cols(C)");
    }
    
    int N = A.get_shape()[0], K = A.get_shape()[1], M = B.get_shape()[1];
    T *a_data = A.get_data(), *b_data = B.get_data(), *c_data = C.get_data();
...
void acc_convolution2D(const Tensor4D<T> &input, const Tensor4D<T> &filters, Tensor4D<T> &output, vector<int> stride) { 
    ...
    if(in_channels != filters.get_shape()[0]) {
        throw("Error: input channels != filter.get_shape()[0]");
    }
    
    if(out_channels != output.get_shape()[1]) {
        throw("Error: output channels != output.get_shape()[1]");
    }
    
    if(batch != output.get_shape()[0]) {
        throw("Error: batch != output.get_shape()[0]");
    }
    
    T *in_data = input.get_data(), *out_data = output.get_data(), *filter_data = filters.get_data();
...
void acc_pad2D(const Tensor4D<T> &input, Tensor4D<T> &output, int padding_left, int padding_right, int padding_top, int padding_bottom) {
    ...
    for(int b = 0; b < B; b++) {
        for(int c = 0; c < C; c++) {
            for(int i = 0; i < N; i++) {
                for(int j = 0; j < M; j++) {
                    out_data[b*padded_N*padded_M * C + c*padded_N*padded_M + (i+padding_top)*padded_M + j + padding_left] = in_data[b*C*M*N + c*M*N + i*M + j];
                }
            }
        }
    }
...
void acc_make_batch(const Tensor4D<T> &inputs, Tensor4D<T> &batch, int batch_start) {
    ...
    if((batch_size > num_inputs) || input_size!=batch_input_size) {
        throw("Error batch,inputs not compatible");
    }
...
void acc_rng(Tensor4D<T> &A, T mtlp) {
    T* a_data = A.get_data();
    int i, istat, n = A.get_shape().get_size(), pn = n;
...
void acc_val(Tensor4D<T> &A, T val) {
    int asize = A.get_shape().get_size();
...
Tensor4D<T> AddVecBroadCast(const Tensor4D<T> &a, const Tensor4D<T> &b) {
    int sizea = a.get_size(), sizeb = b.get_size();
    
    int mxa, mxb, mxc, mxd, ai, aj, ak, al, bi, bj, bk, bl;
    
    if( !(a.a==b.a || ( (a.a==1) ^ (b.a==1) ) ) ) {
        throw("Error: Tensors dim(a) not addition compatible.");
    }
    
    if( !(a.b==b.b || ( (a.b==1) ^ (b.b==1) ) ) ) {
        throw("Error: Tensors dim(b) not addition compatible.");
    }

    if( !(a.c==b.c || ( (a.c==1) ^ (b.c==1) ) ) ) {
        throw("Error: Tensors dim(c) not addition compatible.");
    }

    if( !(a.d==b.d || ( (a.d==1) ^ (b.d==1) ) ) ) {
        throw("Error: Tensors dim(d) not addition compatible.");
    }
...
void AddVecDim0(Tensor4D<T> &A, const Tensor4D<T> &B) {
    if(!(B.get_shape()[1]==1 && B.get_shape()[2]==1 && B.get_shape()[3]==1)) {
        throw("Error: Tensor B is not Vector(1xbx1x1).");
    }

    if(B.get_shape()[0] != A.get_shape()[0]) {
        throw("Error: Tensor A.get_shape()[0] != B.get_shape()[0].");
    }
    
    int sizeA = A.get_shape().get_size(), sizeB = B.get_shape().get_size(), a=A.get_shape()[0], b=A.get_shape()[1], c=A.get_shape()[2], d=A.get_shape()[3];
    int bcd = b*c*d, cd = c*d;
    T *a_data = A.get_data(), *b_data = B.get_data();
...
void AddVecDim1(Tensor4D<T> &A, const Tensor4D<T> &B) {
    ...
    if(B.get_shape()[1] != A.get_shape()[1]) {
        throw("Error: Tensor A.get_shape()[1] != B.get_shape()[1].");
    }
...
void AddVecDim2(Tensor4D<T> &A, const Tensor4D<T> &B) {
    if(!(B.get_shape()[0]==1 && B.get_shape()[1]==1 && B.get_shape()[3]==1)) {
        throw("Error: Tensor B is not Vector(1x1xcx1).");
    }

    if(B.get_shape()[2] != A.get_shape()[2]) {
        throw("Error: Tensor A.get_shape()[2] != B.get_shape()[2].");
    }
...
void AddVecDim3(Tensor4D<T> &A, const Tensor4D<T> &B) {
    if(!(B.get_shape()[0]==1 && B.get_shape()[1]==1 && B.get_shape()[2]==1)) {
        throw("Error: Tensor B is not Vector(1x1x1xd).");
    }
    
    if(B.get_shape()[3] != A.get_shape()[3]) {
        throw("Error: Tensor A.get_shape()[3] != B.get_shape()[3].");
    }
...
void AddVecBroadCast(Tensor4D<T> &A, const Tensor4D<T> &B, int dimension) {
    if(dimension == 0) {
        ...
        throw("Error: dimension to broadcast not valid.");
    }
}
...
template void acc_make_batch(const Tensor4D<double> &, Tensor4D<double> &, int );
